...
export async function getStaticProps({ params }) {
    const postData = await getPostData(params.id, ["seminars", "202425"]);
    return {
        props: {
            postData,
        },
...
export async function getStaticPaths() {
    const paths = getAllPostIds(["seminars", "202425"]);
    return {
        paths,
        fallback: false,
    };
        ...
        <Layout>
            <Head>
                ...
                <meta
                    ...
                    content={
                        postData.abstract
                            ? postData.abstract.trim()
                            : "A course on data practices and service design at Politecnico Milano School of Design, Master in Product Service System Design"
                    }
                />
                <meta itemprop="image" content={postData.image} />
            ...
            <h4>
                {postData.name}, {postData.affiliation}
            </h4>
            <h4>{formattedDate}</h4>
            ...
            <div>
                <Zoom classDialog={"custom-zoom"}>
                    <Image src={postData.image} width={0} height={0} sizes="100%" style={{ width: "100%", height: "auto" }} />
                </Zoom>
            </div>
        </Layout>
    );
